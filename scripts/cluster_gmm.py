...
def main(args):
    ...
    df["Cluster_ID"] = gmm.fit_predict(X.toarray())

    # Raw
    raw_out = args.output.replace(".xlsx", "_raw.xlsx")
    df.to_excel(raw_out, index=False)
    print(f"✅ GMM raw clustering saved to {raw_out}")
    ...
    print(f"✅ GMM with-seed clustering saved to {labeled_out}")
